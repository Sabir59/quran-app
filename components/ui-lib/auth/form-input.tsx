...
interface FormInputProps extends TextInputProps {
  /** Floating label rendered on the top-left border edge (e.g. Login E-Mail field). */
  label?: string;
  rightIcon?: ReactNode;
...
export function FormInput({
  ...
  return (
    ...
    >
      {/* Floating label — sits on the top-left border edge */}
      {label ? (
...
const styles = StyleSheet.create({
  /** Extra top margin so the absolute label has room above the border */
  labeledWrapper: {
    marginTop: 8,
  },
  /** Positioned to sit on the top-left of the input border */
  labelContainer: {
    position: 'absolute',
    top: -10,
    left: 12,
